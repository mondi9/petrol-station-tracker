...
const MapComponent = ({ stations, onStationSelect, onViewDetails, selectedStation, onReportClick, onFindNearest, userLocation, isLocating, onMapClick, nearbyStations = [], travelStats = {} }) => {
    ...
    // Detect live updates
    const lastStationsCount = React.useRef(stations.length);

    // Detect live updates using refs to avoid setState in effect
    useEffect(() => {
        if (stations.length > 0 && lastStationsCount.current > 0) {
            ...
            if (hasRecentUpdate) {
                // Use refs to track the update state without triggering setState in effect
                lastStationsCount.current = stations.length;
                // Schedule the update state changes via callback
                setShowLiveUpdate.currentRef?.();
                setPulseMarkers.currentRef?.();
                const timer = setTimeout(() => {
                    // Reset refs after timeout
                    lastStationsCount.current = stations.length;
                }, 4000);
