...
import { X, User, Edit2, Check, Award, MessageSquare, Activity } from 'lucide-react';
import { updateProfile } from 'firebase/auth'; // We'll need to update authService too if this isn't exposed or access auth directly
// Actually best to do this via a service wrapper, but for now assuming we pass 'user' object from App which is Firebase User
...
const UserProfileModal = ({ isOpen, onClose, user, stats = { contributions: 0, reviews: 0 } }) => {
    ...
    const handleSave = async () => {
        ...
        try {
            ...
            setIsEditing(false);
            // Ideally trigger a refresh of user state in App, but Firebase auth listener should catch it
        } catch (error) {
